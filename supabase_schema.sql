...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  status VARCHAR(50) DEFAULT 'ACTIVE' NOT NULL CHECK (status IN ('ACTIVE', 'SUSPENDED', 'BANNED')),
...
-- 13. Create deposit_methods table (referenced by admin deposit methods CRUD)
CREATE TABLE IF NOT EXISTS public.deposit_methods (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  method_name VARCHAR(100) NOT NULL,       -- e.g. 'Easypaisa', 'JazzCash', 'Bank Transfer'
  bank_name VARCHAR(100) NOT NULL,          -- e.g. 'Easypaisa Mobile Wallet'
  account_title VARCHAR(255) NOT NULL,      -- e.g. 'NOVA OTP SERVICES'
  account_number VARCHAR(100) NOT NULL,     -- e.g. '03001234567'
  instructions TEXT DEFAULT '',             -- Additional payment instructions
  is_active BOOLEAN DEFAULT true NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);

ALTER TABLE public.deposit_methods DISABLE ROW LEVEL SECURITY;

-- 14. Seed default exchange rates in settings (so admin can update dynamically)
INSERT INTO public.settings (key, value)
VALUES 
